...
In section~\ref{sec:chen}, we adjusted the critical value of $T_{CQ}$ under Assumption~\ref{theModel}.
...
This motivates us to propose a new test for hypotheses~\eqref{problem} under Assumption~\ref{theModel}.
...
relies on the subspace $\tilde{\BV}\tilde{\BV}^T$ which is typically unknown.
We shall replace $\tilde{\BV}\tilde{\BV}^T$ by it's estimator.
%Proposition~\eqref{oracleTheorem} implies that the asymptotic null distribution of $T_1$ involves $\sigma^2$.
...
By replacing $\tilde{\BV}\tilde{\BV}^T$ by $\hat{\tilde{\BV}}\hat{\tilde{\BV}}^T$ in the main term of $T_1$, we propose a new test statistic
$$T_2=\|\hat{\tilde{\BV}}^T (\bar{X}_1-\bar{X}_2)\|^2.$$
%As the main part of $T_1$,
%$\|\tilde{\BV}^T (\bar{X}_1-\bar{X}_2)\|^2$ can be directly estimated by $\|\hat{\tilde{\BV}}^T (\bar{X}_1-\bar{X}_2)\|^2$.
In $T_1$, % the term $\|\tilde{\BV}(\bar{X}_1-\bar{X}_2)\|^2$ has expectation $\|\tilde{\BV}(\mu_1-\mu_2)\|^2+\tau(p-r)\sigma^2$,
the bias correction term $n_1^{-1}\mytr(\tilde{\BV}^T \BS_1\tilde{\BV})+n_2^{-1}\mytr(\tilde{\BV}^T \BS_2\tilde{\BV})$ is an unbiased estimator of $\tau (p-r)\sigma^2$, 
and makes $T_1$ an unbiased estimator of $\|\tilde{\BV}(\mu_1-\mu_2)\|^2$.
In particular, $\myE T_1=0$ under the null hypothesis.
However, the bias correction of $T_2$ is more involved.
Under the null hypothesis, 
\begin{equation}\label{touteng3}
\myE T_2=\myE \myE[ T_2 |\BS]=
\tau \myE \big[\mytr(\hat{\tilde{\BV}}^T\bSigma\hat{\tilde{\BV}})\big]
=
\tau (p-r) \sigma^2+
\tau \myE \big[\mytr(\hat{\tilde{\BV}}^T\BV \bLambda \BV^T\hat{\tilde{\BV}})\big].%>\tau (p-r)\sigma^2.
\end{equation}
We would like to correct the two terms of~\eqref{touteng3}.

The first term of~\eqref{touteng3} involves the unknown parameter $\sigma^2$.~\cite{Anderson1986Asymptotic} proved that the MLE of $\sigma^2$ is $\hat{\sigma}^2=(p-r)^{-1}\sum_{i=r+1}^p \lambda_i(\BS)$.
However, the MLE $\hat{\sigma}^2$ tends to be downward biased.
Recently,~\cite{Passemier2015} proposed a bias-corrected estimator
$$
\Big(1+\frac{1}{n-2}\big(r+\hat{\sigma}^2\sum_{i=1}^r \frac{1}{\blambda_i}\big)\Big)\hat{\sigma}^2.
$$
Since $\blambda_i$'s are unknown in our model, we substitute $\blambda_i$ by $\lambda_i(\BS)-\hat{\sigma}^2$ and use
$$
\hat{\sigma}_*^2=\Big(1+\frac{1}{n-2}\big(r+\hat{\sigma}^2\sum_{i=1}^r \frac{1}{\lambda_i(\BS)-\hat{\sigma}^2}\big)\Big)\hat{\sigma}^2
$$
to estimate $\sigma^2$.
 Using Weyl's inequality, we can derive the consistency rate of $\hat{\sigma}^2$ and $\hat{\sigma}_*^2$.
\begin{proposition}\label{varianceEstimation}
    ...
    \hat{\sigma}_*^2=\sigma^2 + O_P\Big(\frac{\max (n,p)}{np}\Big).
    $$
\end{proposition}
\begin{remark}
    The convergence rate of $\hat{\sigma}_*^2$ given in Proposition~\ref{varianceEstimation} is enough for the purpose of this paper.
    We don't claim it is optimal.
\end{remark}

Now we consider the correction of the second term of~\eqref{touteng3}.
Note that
$$
\begin{aligned}
...
\quad \text{and}\quad
\hat{v}_i^T v_i=\frac{1}{\sqrt{1+ \frac{p}{n\blambda_i}\sigma^2}}+O_P(\epsilon_{i,n}),\quad 1\leq i\leq r,
$$
where $\epsilon_{i,n}$ is certain higher order infinitesimal, $i=1,\ldots, r$.
Hence $\mytr(\hat{\tilde{\BV}}^T\BV \bLambda \BV^T\hat{\tilde{\BV}})$
can be approximated by
$$
\sum_{i=1}^r \Big(1-\frac{1}{1+\frac{p}{n\blambda_i}\sigma^2}\Big)\blambda_i
=
\sum_{i=1}^r \frac{p\sigma^2}{n\blambda_i+{p}\sigma^2}\blambda_i.
$$
This motivates us to correct the second term of~\eqref{touteng3} by
$$
\tau\sum_{i=1}^r \frac{p\hat{\sigma}^2}{n\lambda_i(\BS)+{p}\hat{\sigma}^2}\lambda_i(\BS).
$$
...
\begin{appendices}
    \section{Proofs of the results in Section~\ref{sec:chen}}
...
\section{Proofs of the results in Section~\ref{methodology}}
...
\begin{lemma}[\cite{Cai2015Optimal}, Proposition 1]\label{pert}
    ...
    Then
    $$
    \|\BA_1-\BA_2\|\geq \frac{1}{2}(\lambda_r(\BA_1)-\lambda_{r+1}(\BA_2))\|\BV_1 \BV_1^T- \BV_2 \BV_2^T\|.
    $$
\end{lemma}
...
Denote by $\BS=\hat{\BU}\hat{\BE}\hat{\BU}^T$ the spectral decomposition of $\BS$,
    where $\hat{\BU}=(\hat{\BV},\hat{\tilde{\BV}})$ and $\hat{\BE}=\mydiag(\lambda_1(\BS),\ldots,\lambda_p(\BS))$.
...
The sample covariance matrix $\BS$ has the same distribution as the random matrix
$
    {(n-2)}^{-1} \BU \BE^{1/2} \bZ \bZ^T \BE^{1/2} \BU^T
$.
This implies that $\lambda_i(\BS)\sim (n-2)^{-1}\lambda_i(\bZ^T \BE \bZ)$, $i=1,\ldots,r$.
Thus, we only need to consider $(n-2)^{-1}\lambda_i(\bZ^T \BE \bZ)$.
    {\color{red}HERE!!!!!!!!!!!!! IMPROVE THIS !!!!!!!!!!!!!!!}
For $i=1,\ldots,r$, we have
    \begin{equation*}
        \begin{aligned}
            &|\lambda_i(\bZ^T \BE \bZ)-(n-2)(\blambda_i +\sigma^2)|\\
            \leq&
