...
In order to consistently estimate $F(x;\blambda_1,\ldots,\blambda_r,\sigma^2)$, we need to estimate $\blambda_i$, $i=1,\ldots,r$, and $\sigma^2$.
...
The asymptotic normality of $T_2$ is closely related to the convergence rate of $\hat{\tilde{\BV}}\hat{\tilde{\BV}}^T$ to $\tilde{\BV}\tilde{\BV}^T$.
    ...
    In Theorem~\ref{myPanpan}, we assume $p/n^2\to 0$ which is stronger than $p/(p^{\beta}n)\to 0$.
    This is because {\color{red}why?}
...
$$
   ...
   =
   \myE\big[\Pr\big({P_2}>\epsilon{\sigma^2\sqrt{2\tau^2 p}}\big|S\big)\big].
   $$
   Since the conditional probability 
   $
   \Pr\big({P_2}>\epsilon{\sigma^2\sqrt{2\tau^2 p}}\big|S\big)
   $
