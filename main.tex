...
To apply Lemma~\ref{quadraticFormCLT} to $T_{CQ}$, one needs to calculate the eigenvalues of $\BB_n\otimes \bSigma$.
    ...
    %\begin{equation*}
        %\{{(\eta^\top ,\underbrace{0,\ldots,0}_{n_2})}^\top |\, \textrm{$\eta\in \mathbb{R}^{n_1}$ and $\eta^\top  \gamma_1=0$}\}.
    %\end{equation*}
    %The eigenspace corresponding to $-1/n_2(n_2-1)$ is
    %\begin{equation*}
        %\{{(\underbrace{0,\ldots,0}_{n_1},\eta^\top )}^\top | \, \textrm{$\eta\in\mathbb{R}^{n_2}$ and $\eta^\top  \gamma_2=0$}\}.
    %\end{equation*}
    %The eigenvectors corresponding to $(n_1+n_2)/n_1n_2$ and $0$ are respectively
    %\begin{equation*}
        %\Big(-\sqrt{\frac{n_2}{n_1+n_2}}\gamma_1^\top , \sqrt{\frac{n_1}{n_1+n_2}}\gamma_2^\top \Big)^\top 
        %\quad\text{and}\quad
        %\Big(\sqrt{\frac{n_1}{n_1+n_2}}\gamma_1^\top , \sqrt{\frac{n_2}{n_1+n_2}}\gamma_2^\top \Big)^\top .
    %\end{equation*}
    Thus,
    \begin{equation*}
...
%, that is $\tilde{\BV}_k^\top  \BV_k=\BO_{r_k\times(p-r_k )}$
...
%\begin{theorem}\label{Chenstheory}
 %Under Assumption~\ref{theModel},
    ...
    %$(\tau p^{\beta})^{-1/2}\big(\BV^\top  (\mu_1-\mu_2)\big)_i\to \zeta_i\in(-\infty,+\infty)$, $i=1,\ldots,r$, and
    %${(\tau p^\beta)}^{-1}\|\tilde{\BV}^\top  (\mu_1-\mu_2)\|^2\to \zeta^*\in [0,+\infty)$. We have
    %$$
        %\frac{1}{\tau p^{\beta}} T_{CQ}
...
%\zeta^*+
    %\sqrt{2}\sigma^2 \delta_{\{\frac{1}{2}\}}(\beta)\epsilon
        ...
        %\sum_{i=1}^r \omega_i,
    %$$
%where $Z_{1},\ldots,Z_{r}$ and $\epsilon$ are i.i.d.\ $\mathcal{N}(0,1)$ random variables, $\delta_{\{\frac{1}{2}\}}(\beta)$ equals to $1$ if $\beta=1/2$ and $0$ otherwise.
%\end{theorem}
...
\begin{aligned}
    ...
    \\
    %&=\frac{\sum_{i\neq j}^{n_1}X_{1i}^\top \tilde{\BV}\tilde{\BV}^\top  X_{1j}}{n_1(n_1-1)}+\frac{\sum_{i\neq j}^{n_2}X_{2i}^\top \tilde{\BV}\tilde{\BV}^\top  X_{2j}}{n_2(n_2-1)}-2\frac{\sum_{i=1}^{n_1}\sum_{j=1}^{n_2}X_{1i}^\top \tilde{\BV}\tilde{\BV}^\top  X_{2j}}{n_1n_2}
\end{aligned}
\end{equation*}
...
%By replacing $\tilde{\BV}\tilde{\BV}^\top $ by $\hat{\tilde{\BV}}\hat{\tilde{\BV}}^\top $, the main term $\|\tilde{\BV}^\top  (\bar{X}_1-\bar{X}_2)\|^2$ of $T_1$ becomes
...
\tau \mytr(\hat{\tilde{\BV}}^\top \bSigma\hat{\tilde{\BV}})
        =
\tau (p-r) \sigma^2+
\tau \mytr(\hat{\tilde{\BV}}^\top \BV \bLambda \BV^\top \hat{\tilde{\BV}}).%>\tau (p-r)\sigma^2.
    \end{aligned}
\end{equation*}
Note that
\begin{equation*}
...
%While ${n_k^{-1}}\mathrm{tr}(\tilde{\BV}^\top  S_k\tilde{\BV})$ can be estimated by ${n_k^{-1}}\mathrm{tr}(\hat{\tilde{\BV}}_k^\top  S_k\hat{\tilde{\BV}}_k)$, $k=1,2$.
%Define
%\begin{equation*}
%\begin{aligned}
    %T_2&=\|\hat{\tilde{\BV}}^\top (\bar{X}_1-\bar{X}_2)\|^2-\frac{1}{n_1}\mathrm{tr}(\hat{\tilde{\BV}}_1^\top  S_1\hat{\tilde{\BV}}_1)-\frac{1}{n_2}\mathrm{tr}(\hat{\tilde{\BV}}_2^\top  S_2\hat{\tilde{\BV}}_2).
    %T_2=\frac{\sum_{i\neq j}^{n_1}X_{1i}^\top \hat{\tilde{\BV}}\hat{\tilde{\BV}}^\top  X_{1j}}{n_1(n_1-1)}+\frac{\sum_{i\neq j}^{n_2}X_{2i}^\top \hat{\tilde{\BV}}\hat{\tilde{\BV}}^\top  X_{2j}}{n_2(n_2-1)}
%-2\frac{\sum_{i=1}^{n_1}\sum_{j=1}^{n_2}X_{1i}^\top \hat{\tilde{\BV}}\hat{\tilde{\BV}}^\top  X_{2j}}{n_1n_2}
%\end{aligned}
%\end{equation*}
...
The asymptotic normality of $T_2$ is closely related to the convergence rate of $\hat{\tilde{\BV}}\hat{\tilde{\BV}}^\top $ to $\tilde{\BV}\tilde{\BV}^\top $.
    ...
    %That is, the convergence rate of $\hat{\tilde{\BV}}\hat{\tilde{\BV}}^\top $ to $\tilde{\BV}\tilde{\BV}^\top $ should be faster than $\sqrt{p}/p^\beta$.
...
%And when $\beta\geq \frac{3}{4}$, our assumption is weaker than~\cite{2016arXiv160202491A}'s. Note that when $\beta=\frac{1}{2}$, $\frac{\sqrt{p}}{n_1+n_2}$ is a necessary condition to make $\hat{\BV}\hat{\BV}^\top $ a consistent
%estimator of $\BV\BV^\top $ (see lemma 2 in appendix). So condition $\frac{\sqrt{p}}{n_1+n_2}$ is roughly the best we can do if the relationship between $p$ and $n$ doesn't involve $\beta$.
%\end{remark}
...
%In fact, our asymptotic results are still valid with a consistently estimated $r$.
...
%Now we consider the simulation of the empirical level. 
...
%Table~\ref{realTable} lists the $p$ values of the proposed test and competing tests.
%For $\alpha=0.05$, the proposed test rejects the null hypothesis while all other tests can not reject the null hypothesis.
%Hence the proposed test implies that Monday effect exists in China stock market, which is a conclusion different from~\cite{Ma2015A}.

%\begin{table}[ht]
%\caption{$p$-values of the proposed test and competing tests for stock data.} 
%\label{realTable}
%\vspace{3mm}
%\centering
%\begin{tabular}{llllllllll}
%\toprule
    %NEW& CCQ & FAST & CQ & SD & LJW\\
%\midrule
    %0.0009 & 0.0963 & 0.0710 & 0.1962 & 0.2143 & 0.2902\\
%\bottomrule
%\end{tabular}
%\end{table}
...
%For non-normal distributions, the distribution of $\|\hat{\tilde{\BV}}^\top (\bar{X}_1-\bar{X}_2)\|^2$ under the null distribution is hard to evaluate.
%As a result, it's hard to standardize $\|\hat{\tilde{\BV}}^\top (\bar{X}_1-\bar{X}_2)\|^2$ to formulate a test with asymptotically level $\alpha$.
...
%    let $X$ be a $p$-dimensional random vector with distribution $\mathcal{N}(0,\bSigma)$. Denote the spectral decomposition of $\bSigma$ by $\bSigma =\sum_{i=1}^p \lambda_i p_i p_i^\top $ with $\lambda_1\geq \cdots \geq \lambda_p$. Then $X^\top  p_i p_i^\top  X$ is stochastically larger than $X^\top  p_j p_{j}^\top  X$ for $i<j$.
%\end{lemma}
%\begin{proof}[\textbf{Proof}]
%    The lemma is established immediately once we note that $X^\top  p_i p_i^\top  X/\sqrt{\lambda_i}$ is distributed as $\chi^2$ distribution with freedom $1$.
%\end{proof}
...
\begin{proof}[\textbf{Proofs of Theorems~\ref{Chenstheory1} and~\ref{Chenstheory2}}]
    ...
    %\end{aligned}
    %$$
    %where $\epsilon\sim \mathcal{N}(0,1)$. By law of large numbers, we have $W_i/(n_i-1)\xrightarrow{P}1$. Hence
%$$
%\begin{aligned} 
...
\zeta^*+
    ...
    %$$
    %\frac{1}{\sqrt{2(p-r)}}\big( W-(p-r)\big)\xrightarrow{\mathcal{L}}\mathcal{N}(0,1).
    %$$
%
...
%Denote by $\BS=\hat{\BU}\hat{\BE}\hat{\BU}^\top $ the spectral decomposition of $\BS$, where $\hat{\BU}=(\hat{\BV},\hat{\tilde{\BV}})$ and $\hat{\BE}=\mydiag(\lambda_1(\BS),\ldots,\lambda_p(\BS))$.
    Let $\bZ$ be a $p\times n$ random matrix with i.i.d. $\mathcal{N}(0,1)$ entries.
...
\myE \|\hat{\BV}\hat{\BV}^\top -\BV\BV^\top \|_F^2 =O(\frac{p}{p^{\beta}n}),
    ...
    %Note that the rank of $\hat{\BV}\hat{\BV}^\top -\BV\BV^\top $ is at most $2r$. 
...
$$
    ...
    %Let $Y_{k,i}=\tilde{\BV}^\top  (X_{k,i}-\mu_k)$, $i=1,\ldots,n_k$, $k=1,2$.
    %Then $Y_{k,i}\sim \mathcal{N}(\tilde{\BV}^\top \mu_k,\sigma^2 \BI_{p-r})$.
    ...
    %\begin{equation}\label{prop1eq1}
        %\begin{aligned}
            %&\|\tilde{\BV}^\top (\bar{X}_1-\bar{X}_2)\|^2
            %=\|\tilde{\BV}^\top (\mu_1-\mu_2)+(\bar{Y}_1-\bar{Y}_2)\|^2\\
            %=&\|\tilde{\BV}^\top (\mu_1-\mu_2)\|^2+\|\bar{Y}_1-\bar{Y}_2\|^2+
            %2{(\mu_1-\mu_2)}^\top \tilde{\BV}(\bar{Y}_1-\bar{Y}_2)\\
            %=&\|\tilde{\BV}^\top (\mu_1-\mu_2)\|^2+\|\bar{Y}_1-\bar{Y}_2\|^2+
            %o_P(\frac{\sqrt{p}}{n}).
        %\end{aligned}
    %\end{equation}
    ...
    %\begin{equation*}
        %\begin{aligned}
    %&2{(\mu_1-\mu_2)}^\top \tilde{\BV}(\bar{Y}_1-\bar{Y}_2)\sim \mathcal{N}(0,4\sigma^2 \tau \|\tilde{\BV}^\top (\mu_1-\mu_2)\|^2)\\
            %=& O_P(\sqrt{\tau}\|\tilde{\BV}^\top (\mu_1-\mu_2)\| )=o_P(\frac{\sqrt{p}}{n}).
        %\end{aligned}
    %\end{equation*}
...
%\begin{equation*}
    %\begin{aligned}
        %&\frac{\|\hat{\tilde{\BV}}^\top (\bar{X}_1-\bar{X}_2)\|^2-\|\tilde{\BV}^\top (\mu_1-\mu_2)\|^2
        %-\tau (p-r)\sigma^2
        %}{\sqrt{2\tau^2 p\sigma^4}}
        %\\
        %=&\frac{1}{\sqrt{2\tau^2 p\sigma^4}}(
        %P_1+P_2+P_3
        %),
    %\end{aligned}
%\end{equation*}
...
\begin{align*}
    ...
    %P_3&=\|\hat{\tilde{\BV}}^\top (\mu_1-\mu_2)\|^2-\|\tilde{\BV}^\top (\mu_1-\mu_2)\|^2.
\end{align*}
...
%$$
%\rho\Big(\mathcal{L}\Big(\frac{\|\hat{\tilde{\BV}}^\top \big((\bar{X}_1-\mu_1)-(\bar{X}_2-\mu_2)\big)\|^2-\tau\mathrm{tr}(\hat{\tilde{\BV}}^\top \bSigma\hat{\tilde{\BV}})}{\sqrt{2\tau^2\mathrm{tr}(\hat{\tilde{\BV}}^\top \bSigma\hat{\tilde{\BV}})^2}}\Big|S\Big),\mathcal{N}(0,1)\Big)\xrightarrow{P} 0.
%$$
$$
...
$$
Then we have 
% Proposition~\ref{varianceEstimation} and Theorem~\ref{myPanpan} imply that
...
%\end{remark}
    ...
    %Since the new test statistic estimates $\|\tilde{\BV}^\top (\mu_1-\mu_2)\|^2$,
 %the superiority of the new test will be established if 
    
%\begin{equation}\label{yuedengyu}
    %\frac{\|\tilde{\BV}^\top (\mu_1-\mu_2)\|}{\|\mu_1-\mu_2\|}\approx 1.
%\end{equation}
%Obviously,~\eqref{yuedengyu}
%is not always the case since there always exists some
%$\tilde{\BV}$ and $\mu_1-\mu_2$ such that $\|\tilde{\BV}^\top (\mu_1-\mu_2)\|=0$.
%However,~\eqref{yuedengyu} is reasonable since $\tilde{\BV}\tilde{\BV}^\top $ is nearly an identity matrix in the sense that
    %${\|\BI_p-\tilde{\BV}\tilde{\BV}^\top \|_F^2}/{\|\BI_p\|_F^2}=r/p\to 0$. 
