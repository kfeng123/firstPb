...
\frac{(p-r)/p}{\sqrt{2\tau^2((p-r)\sigma^4)}}.
    \end{aligned}
\end{equation*}
Thus, for sufficiently large $p$,~\eqref{touteng2} is maximized when $k=p-r$.

As we have shown,~\eqref{touteng} is maximized when $\BO=\tilde{V}$. We define the following variable
\begin{equation*}
\begin{aligned}
    T_{1}&=\|\tilde{\BV}^T(\bar{X}_1-\bar{X}_2)\|^2-\frac{1}{n_1}\mathrm{tr}(\tilde{\BV}^T \BS_1\tilde{\BV})-\frac{1}{n_2}\mathrm{tr}(\tilde{\BV}^T \BS_2\tilde{\BV}).
    \\
...
relies on the subspace $\tilde{\BV}\tilde{\BV}^T$ which is unknown. Thus, we estimate $\tilde{\BV}\tilde{\BV}^T$ by its sample counterpart.
...
\begin{proof}[\textbf{Proves of Theorem~\ref{Chenstheory1} and Theorem~\ref{Chenstheory2}}]
    ...
    Since $(n_k-1)\BS_k\sim \text{Wishart}_p(n_k-1,\bSigma)$, $k=1,2$, we have %$(n_1-1)\mytr S_1\sim \sum_{i=1}^{p} \lambda_i (\bSigma) W_i$ where $W_i\sim \chi^2_{n_1-1} $.
