...
\begin{abstract}
    ...
    The covariance matrices are assumed to be spiked, which often arises in practice. 
...
\end{equation}
adopted by~\cite{Chen2010A} is violated when $\Sigma$ has uniform correlation structure, that is, $\Sigma=(1-\rho)I_p+\rho\textbf{1}_p \textbf{1}_p^T$ where $0<\rho<1$, $I_p$ is the $p$ dimensional identity matrix and $\bm{1}_p$ is the $p$ dimensional vector  with elements $1$.
...
as $p\to \infty$.
 ...
 \end{equation}
 where $\Lambda=\mydiag(\lambda_1,\ldots,\lambda_r)$, $\lambda_1\geq \cdots\geq \lambda_r>0$, $r$  is a known fixed number, $V$ is a $p\times r$ orthonormal matrix and $\sigma^2>0$.
...
In section~\ref{sec:chen}, we derived the asymptotic distribution of $T_{CQ}$ under spiked covariance model. 
...
\begin{lemma}\label{quadraticFormCLT}
    ...
    \end{equation}
    where ``$\xrightarrow{\mathcal{L}}$" means convergence of a sequence of random variables in law.
\end{lemma}
...
It is possible to adjust the critical value of $T_{CQ}$ such that the resulting test has correct level $\alpha$ asymptotically.
...
+
        ...
        \sum_{i=1}^r \lambda_i
        $.
By Theorem~\ref{Chenstheory1} and Theorem~\ref{Chenstheory2}, if the critical value of $\tau^{-1}T_{CQ}$ is defined as the upper $\alpha$ quantile of
$F(x;\lambda_1,\ldots,\lambda_r,\sigma^2)$, the resulting test has level $\alpha$ asymptotically.
        However, the distribution $F(x;\lambda_1,\ldots,\lambda_r,\sigma^2)$ involves some nuisance parameters.
...
\begin{proposition}\label{eigenconsis}
    Under Assumption~\ref{theModel}, suppose $n=O(p)$ and $p=o(np^{\beta})$, then
    $$
...
By Theorem~\ref{Chenstheory1} and~\ref{Chenstheory2}, the adjusted $T_{CQ}$ has trivial asymptotic power if $\zeta_1=\cdots=\zeta_r=\zeta^*=0$, which is equivalent to $\sum_{i=1}^r \zeta_i^2 +\zeta^*=0$, or
$$
...
$$
and $\mathcal{L}(Z_n|S)$ is the conditional distribution of $Z_n$ given $S$.
