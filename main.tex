...
as $p\to \infty$.
 ...
 \end{equation}
 where $\bLambda=\mydiag(\lambda_1,\ldots,\lambda_r)$, $\lambda_1\geq \cdots\geq \lambda_r>0$, $r\geq 1$, $\BV$ is a $p\times r$ orthonormal matrix and $\sigma^2>0$.
...
Then Lemma~\ref{quadraticFormCLT} implies that under the null hypothesis, the condition~\eqref{chenscondition} is a necessary and sufficient condition for 
   ...
   In a class of applications, however, the correlations between variables are mainly driven by several common factors so that $\bSigma$ has a few eigenvalues which are much larger than the others.
...
\begin{assumption}\label{theModel}
    The covariance matrix $\bSigma$ satisfies $ 
\bSigma=\BV\bLambda \BV^T+\sigma^2 \BI_p
$, where $\bLambda=\mydiag(\lambda_{1},\ldots,\lambda_{r})$, 
 $\lambda_{1}\geq \cdots \geq \lambda_{r}>0$, $r$ is a known number,
$\BV$ is  a $p\times r$ orthonormal matrix and $\sigma^2>0$.
...
The column space of $\BV$,  the eigenspace of $\bSigma$ corresponding to the leading eigenvalues, is called principal space. Since $\BV$ is an orthonormal matrix, $\BV \BV^T$ is the orthogonal projection matrix on the principal space.
Let $\tilde{\BV}$ be a $p\times (p-r)$ full column rank orthonormal matrix orthogonal to columns of  $\BV$.
%, that is $\tilde{\BV}_k^T \BV_k=\BO_{r_k\times(p-r_k )}$
 Although such $\tilde{\BV}$ is not unique, the projection matrix  $\tilde{\BV}\tilde{\BV}^T=\BI_p-\BV \BV^T$ is unique and is the projection matrix on the orthogonal complement of principal space.
...
\begin{proof}[\textbf{Proof of Lemma~\ref{quadraticFormCLT}}]
    ...
    We have $c_{ni}\in[-\sqrt{2}/2,\sqrt{2}/2]$.
    For $t\in (-1,1)$, we have
    \begin{equation*}
        \begin{aligned}
            &\log \myE\big[\exp{\big(it \sum_{j=1}^{k_n}c_{nj}(Z_{nj}^2-1)\big)}\big]
            =
