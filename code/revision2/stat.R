...
# New test final version 2
myStatFinal2 <- function(X1, X2, n1, n2, ...) {
    r <- list(...)$r
    p <- ncol(X1)
    n <- n1+n2-2
   ...
   if(is.null(list(...)$myEigen)){
    ...
    # variance estimator
    oldSigmaSqEst <- mean(myEigen$values[-(1:r)])
    newSigmaSqEst <- (1-r/n)^(-1) * oldSigmaSqEst
    # eigenvalue estimator
    myEigenEstimator <- myEigen$values[1:r]-((p+n-r)/n)*newSigmaSqEst
    
    tau <- 1 / n1 + 1 / n2
    stat <-
        sum((t(myTildeV) %*% (colMeans(X1) - colMeans(X2))) ^ 2) -
        tau*(p-r)*newSigmaSqEst -
        tau * sum(p*newSigmaSqEst*myEigenEstimator/(n*myEigenEstimator+(n+p)*newSigmaSqEst))
    
    # studentized statistic
    statVVV <- 2* tau^2*(
        sum((p*newSigmaSqEst*myEigenEstimator/(n*myEigenEstimator+(n+p)*newSigmaSqEst))^2)
        +
        2*newSigmaSqEst*sum(p*newSigmaSqEst*myEigenEstimator/(n*myEigenEstimator+(n+p)*newSigmaSqEst))
        +
        newSigmaSqEst^2*(p-r)
    )
    studentStat <- stat / sqrt(statVVV)
